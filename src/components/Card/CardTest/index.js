...
function CardTest({info, onClickButton_1, onClickButton_2, title_1, title_2 }) {
    return ( 
        <View className="h-[115px] bg-yellow-2 rounded-20 mt-5 flex flex-row justify-between px-5"  style={{elevation:10}}>
            <View className="flex flex-1 justify-between h-full">
                <Text className="font-interRegular">Tên: {info.title}</Text>
                <Text className="font-interRegular">Mã bài: MA</Text>
