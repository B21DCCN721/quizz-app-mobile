...
function SigupTeacherScreen({navigation}) {
    ...
    return (  
        ...
        <View className="flex flex-1 px-5 mt-20">
          <Text className="text-2xl font-interSemiBold my-5">Đăng ký tài khoản</Text>
          {/* Input Email */}
          <View className="mt-5">
            <Text className="font-interSemiBold my-2">Email</Text>
            <Input value={email} placeholder="abc@gmail.com" onChange={setEmail}/>
          </View>
            {/* Input name */}
          <View className="mt-5">
          ...
          <Button title="Đăng ký" sxButton="bg-red mt-5" sxText="text-center text-white font-interBold" onClick={() => navigation.replace("Start")} />
        </View>
      </DefaultLayot>
    );
}
